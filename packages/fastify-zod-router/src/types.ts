...
type InferOrUnknown<T extends AnyZod | undefined> = T extends AnyZod
  ? T["_output"]
  : unknown;
...
> = TResponse extends AnyZod
  ...
  : TResponse extends Record<string | number, AnyZod>
    ? {
        [K in keyof TResponse]: TResponse[K] extends AnyZod
          ? TResponse[K]["_output"]
          : never;
      }[keyof TResponse]
    : unknown;

export type RouteGenericFromSchema<
  TSchema extends RouteSchema
> = RouteGenericInterface & {
...
> {
  ...
  register: (
    app: FastifyInstance,
    options?: RouterRegisterOptions
  ) => Promise<void> | void;
}

export interface RouterBuilder<
  TRoutes extends readonly RouteDefinition[] = readonly []
> {
  route<
    TPath extends string,
    TMethod extends HttpMethod,
  ...
  options<TPath extends string, TSchema extends RouteSchema>(
    ...
    [...TRoutes, RouteDefinitionWith<"OPTIONS", TPath, TSchema>]
  >;
