...
3. **Run the CLI from the tarball:**
   ...
   When the interactive UI appears, use ↑/↓ to navigate, press space to toggle optional modules, and choose **skip** when asked to install dependencies (the `@fast-next/*` packages are local-only until we publish). Because the tarball version stays `0.0.0`, clear `~/.npm/_npx` or delete the extracted temp folder before re-running `npx` so you don’t accidentally execute an older cached build.

> **Already published?** If you just want to try the public release, skip the tarball entirely:
> ```bash
> pnpm dlx create-fast-next@latest init my-app
> ```
> The UI is the same shadcn-style flow — pick directories, choose a database, select the ORM, then toggle optional modules from the multi-select. Use `--yes` when you need a fully scripted run.
