...
| --- | --- | --- |
| `--app <path>` | Location of the Next.js `app` directory. | `app` |
| `--server <path>` | Directory for Fastify files (`fastify-app.ts`, `routes`, etc.). | `src/server` |
| `--api <path>` | Catch-all API route (relative to project root). | `<app>/api/[...fastify]` |
| `--install <pm>` | Install deps with `pnpm`, `npm`, `yarn`, `bun`, `auto`, or `skip`. | Prompt (TTY) / `skip` (CI) |
| `--orm <none|prisma|drizzle>` | Generate ORM boilerplate. | Prompt (TTY) / `none` |
| `--db <sqlite|postgres|mysql>` | Database driver when ORM is enabled. | Prompt (TTY) / `sqlite` |
| `--with-queue` | Add BullMQ service, queue, workers. | Prompt |
| `--with-cache <provider>` | Add cache service (`memory`, `redis`, `upstash`). | Prompt |
| `--with-mcp` | Add MCP server, sample tool, Fastify routes. | Prompt |
| `--with-docker` | Add `docker-compose.yml`, Dockerfiles, `.env.example`, and npm scripts. | Prompt |
| `--force` | Overwrite existing files. | `false` |
| `--yes` | Skip prompts entirely (great for AI/CI). | `false` |
