...
## Interactive Flow at a Glance

When you run `pnpm dlx create-fast-next@latest init`, the CLI now opens a shadcn-style session:

```text
? Project directory · .
? Next.js app directory · app
? Server directory · src/server
? API catch-all path · app/api/[...fastify]
? Select database ▸ sqlite
? Select ORM ▸ drizzle
❯◉ BullMQ queues & workers
 ◉ Cache service (memory/redis/upstash)
 ◉ Better Auth integration
 ◉ MCP server scaffold
 ◉ Docker & docker-compose templates

(Use arrow keys to move, space to toggle, enter to continue.)
```

This keeps all optional modules in one multi-select (exactly like the shadcn UI): arrow keys move, space toggles, enter confirms. Skip interactivity by passing flags (e.g. `--with-auth`) or `--yes`.
...
- `--with-auth` → Generate Better Auth service, Next.js catch-all route, and Fastify plugin wiring.
...
- **Interactive tip** – Without `--yes` every prompt is arrow-driven: pick the database first, then the ORM, and use the multi-select (↑/↓ + space) to toggle queue/cache/auth/MCP/Docker before hitting enter. Flip `--yes` (or pass flags) if you need a fully non-interactive run.

- **Core bridge** – Catch-all API route, `src/server/fastify-app.ts`, the full `src/server/http/{routes,server}` tree (with FAST_NEXT markers), and `src/server/api.ts` + browser client wiring.
...
- **Better Auth** – `services/auth/better-auth.ts`, `app/api/auth/[...betterAuth]/route.ts`, Fastify plugin registration, and docs pointing to configuration steps.
- **MCP** – `services/mcp/mcp.service.ts`, standalone server entry, `features/mcp/tools` with a sample tool/index, plus Fastify routes listing MCP tools.
...
- **tsconfig alias** – Ensures `@/*` points to both `src/*` and project root so the generated imports resolve immediately.
...
Leverage these commands in automation (or AI workflows) by combining flags: e.g. `pnpm dlx create-fast-next init . --yes --orm prisma --db postgres --with-auth --with-queue --with-cache redis --with-mcp --with-docker --install auto`.
