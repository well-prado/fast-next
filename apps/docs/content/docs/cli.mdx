...
- **Interactive tip** – When running without `--yes`, the CLI opens a single multi-select for optional modules. Use `↑`/`↓` to navigate, press `space` to toggle queue/cache/auth/MCP/Docker, and hit `enter` to confirm.
