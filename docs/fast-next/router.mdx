...
Fastify consumes the Zod schemas directly via `fastify-type-provider-zod`, so you can reuse the exact same schema instances for tooling or shared validation.
