...
await api.users.get.query({ params: { id: "1" } });
await api.projects.list.query();
...
`createServerClient` iterates the Fastify route metadata and creates ergonomic chains (`api.resource.operation.query|mutate`). Because the structure is derived from the routes themselves, editors autocomplete the available endpoints automatically.
