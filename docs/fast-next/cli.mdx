...
- `docker-compose.yml` with app, Postgres, Redis, and MCP services.
...
pnpm dlx create-fast-next queue status
```
...
- `status` currently prints a reminder to inspect BullMQ dashboards; future releases can integrate Redis metrics.
- `--dir` and `--entry` mirror other commands so you can target different folders.
